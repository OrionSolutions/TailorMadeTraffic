...
VP_Security::instance()->whitelist_function('vp_dep_is_embed_code');

function vp_dep_is_embed_code($value)
{
	if($value === 'embed_code')
		return true;
	return false;
...
function vp_font_preview($face, $style, $weight, $size, $line_height)
{
	...
	$link  = reset($links);
	$dom   = '<link href="$link" rel="stylesheet" type="text/css"><p style="padding: 0 10px 0 10px; font-family: $face; font-style: $style; font-weight: $weight; font-size: {$size}px; line-height: {$line_height}em;">Grumpy wizards make toxic brew for the evil Queen and Jack</p>';
